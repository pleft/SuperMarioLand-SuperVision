...
TRACKS = [0x07, 0x04, 0x0C, 0x01, 0x09, 0x0A, 0x0D, 0x03, 0x0B, 0x0F, 0x12]
NAMES  = ["MUS_LEVEL", "MUS_UNDER", "MUS_STAR", "MUS_GOAL", "MUS_BONUS", "MUS_BWALK", "MUS_BAWARD",
          "MUS_T03", "MUS_BOSS", "MUS_RESCUE", "MUS_REVEAL"]
...
for tr in TRACKS:
    ...
    track_lists[tr] = L
    dest = out2 if tr in FIXED_TRACKS else out
    track_base[tr] = ("music2_data" if tr in FIXED_TRACKS else "music_data", len(dest))
    dest += tout
    inc.append(f"MUS_T{tr:02X}_LT = {ltab_off + BIAS}")
    print(f"track ${tr:02X}: {len(tout)}B -> {track_base[tr][0]}+{track_base[tr][1]}"
          f" lists@{L} ({len(order)} phrases)")
...
print(f"music.bin: {len(out)} bytes (LEVELS prefix), music2.bin: {len(out2)} bytes (FIXED)")
