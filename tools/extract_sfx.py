...
def main():
    ...
    wanted = [("dfe0", 0xDFE0, v) for v in (1, 2, 3, 4, 5, 6, 7, 8, 9, 0x0A, 0x0B)] + \
             [("dff0", 0xDFF0, 1)] + \
             [("dff8", 0xDFF8, v) for v in (1, 2, 3, 4)] + \
             [("dfe8", 0xDFE8, v) for v in (2,)]     # the death jingle; more with the music
    blob = bytearray(); aux = bytearray(); table = []; masks = []; nctrl = []
    # The LEVELS prefix (shared by every level bank) is full: bank 5 has no room
    # for one more stream. These go to FIXED's tail (main.s sfx_aux); the player
    # adds a 16-bit offset to sfx_data, so a far stream costs nothing at runtime.
    FAR_SFX = set()   # (was {"dfe0_09"} while FIXED had room; W2FARM moved to $A650 instead)
    for name, mb, v in wanted:
        ...
        if label in FAR_SFX:            # lives in FIXED (sfx_aux), not the prefix
            table.append((label, ("aux", len(aux))))
            aux += enc
        else:
            table.append((label, len(blob)))
            blob += enc
        print(f"SFX {label}: {len(enc)} bytes{' (aux)' if label in FAR_SFX else ''}")
    with open(os.path.join(OUT, "sfx.bin"), "wb") as f:
        f.write(bytes(blob))
    with open(os.path.join(OUT, "sfx_aux.bin"), "wb") as f:
        f.write(bytes(aux))
    with open(os.path.join(OUT, "sfx.inc"), "w") as f:
        f.write("; GENERATED by tools/extract_sfx.py (build artifact, rule 5)\n")
        live = [(l, o) for l, o in table if not isinstance(o, int) or o >= 0]
        def off(o):                 # aux streams: a link-time offset from sfx_data
            return f"(sfx_aux + {o[1]} - sfx_data)" if isinstance(o, tuple) else str(o)
        ...
        for l, o in live: f.write(f"    .byte <{off(o)}\n")
        ...
        f.write(".endif\n")
    print(f"total: {len(blob)} bytes (+{len(aux)} aux in FIXED), {len(live)} live SFX")
