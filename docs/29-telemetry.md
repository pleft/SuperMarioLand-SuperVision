...
Rules: write MAGIC..payload first, COMMIT last, once per frame. The whole
list is rewritten every frame (full snapshot), so a missed/corrupt byte
self-heals next frame — the link needs no ACKs.
