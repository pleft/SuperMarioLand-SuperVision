...
frame_flag = $60                 ; scratch ZP

nmi:                             ; the PORT's real NMI body
    pha
    inc frame_flag
    stz $2002                    ; XSCROLL = 0 (HUD rows)
    lda #16                      ; arm the line-16 timer IRQ
    sta $2023
    pla
    rti

irq:                             ; the PORT's real IRQ body
    pha
    lda #0
    sta $2002                    ; playfield scroll (0 here)
    lda $2024                    ; read clears the timer IRQ
    pla
    rti
